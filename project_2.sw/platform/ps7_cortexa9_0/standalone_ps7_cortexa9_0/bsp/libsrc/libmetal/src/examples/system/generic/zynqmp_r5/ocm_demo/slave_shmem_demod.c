 ...
 */
  ...
  *  3. Check the demo control value in the shared memory to wait for RPU1
  *     to start the demo.
 ...
 *        Wait for message from RPU1. Once received, read and echo it back.
 *
...
static int shmem_echod(struct metal_io_region *shm_io)
{
	...
	xil_printf("SLAVE> Wait for Master.\r\n");
	while (metal_io_read32(shm_io, SHM_DEMO_CNTRL_OFFSET) !=
		DEMO_STATUS_START);

	xil_printf("SLAVE> Demo has started.\r\n");
	...
	xil_printf("SLAVE> OCM Shared memory test finished\r\n");
	return 0;
}
